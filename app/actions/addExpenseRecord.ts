...
async function addExpenseRecord(formData: FormData): Promise<RecordResult> {
  ...
  let userExists = await db.user.findUnique({ where: { clerkUserId: userId } });
  if (!userExists) {
    // If not, create the user with minimal info
    userExists = await db.user.create({
      data: {
        clerkUserId: userId,
        email: '', // You can fetch email from Clerk if available
        name: '',
        imageUrl: '',
      },
    });
